          ...
          <table className="w-full border-collapse text-left sm:min-w-[640px]">
            <thead>
              <tr className="border-b border-rule font-mono text-[11px] uppercase tracking-[0.06em] text-ink-3">
                ...
                <th className="hidden w-32 px-4 py-2.5 font-normal sm:table-cell">Produk</th>
                <th className="hidden w-36 px-4 py-2.5 font-normal sm:table-cell">Status</th>
                <th className="hidden w-36 whitespace-nowrap py-2.5 pl-4 pr-6 text-right font-normal sm:table-cell lg:pr-10">Diperbarui</th>
              </tr>
            </thead>
            <tbody>
              {visible.map((item) => (
                <tr key={item.id} className="group relative border-b border-rule hover:bg-panel">
                  <td className="py-3 pl-6 pr-6 sm:pr-4 lg:pl-10">
                    <Link
                    ...
                    <div className="mt-0.5 line-clamp-1 max-w-[60ch] text-[13px] text-ink-2">{item.brief}</div>
                    <div className="mt-1.5 flex items-center gap-3 text-[12px] text-ink-2 sm:hidden">
                      <ContentStatusTag status={item.status} />
                      <span>{productName(item.product)}</span>
                      <span className="ml-auto font-mono text-ink-3">{formatRelative(item.updated_at)}</span>
                    </div>
                  </td>
                  <td className="hidden px-4 py-3 text-[13px] sm:table-cell">{productName(item.product)}</td>
                  ...
                  <td
                    className="hidden whitespace-nowrap py-3 pl-4 pr-6 text-right sm:table-cell font-mono text-[12px] text-ink-2 lg:pr-10"
